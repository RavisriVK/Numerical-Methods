...
int main(int argc, char **argv)
{
    ...
    FILE *Randomness=fopen("RandomCheck.dat","w");
    ...
    /*Calculating hamming distances for different offsets*/
    for(int i=0; i<MaxOffset; i++)
    {
        HammingDistance=0;
        /*Hamming distance using bitwise operators for efficiency*/
        for(int j=0; j<p; j++)
        {
            if(BitStore[1][j]^BitStream[i+BitStore[0][j]])
                HammingDistance++;
        }
        /*Initializing MinHammingDistance*/
        if(i==0)
        {
            MinHammingDistance=HammingDistance;
            OffsetLocation=0;
        }
        /*Finding minimum hamming distance and corresponding offset*/
